...
import { workflowData } from "@/data/loaders/digital_design";
...
export default async function IndustryPage({ params, searchParams }: PageProps) {
  ...
  if (slug.includes("_")) {
    ...
    redirect(`/services/design/${toCanonicalSlug(slug)}${langSuffix}`);
  }
  ...
  return (
    <>
      ...
      <div className="overflow-x-hidden" dir={isRTL ? "rtl" : "ltr"}>
        ...
        <Contact serviceName={data.hero_section.title} sourcePage={`/services/design/${toCanonicalSlug(slug)}`} />
      </div>
      <Footer />
    </>
  );
