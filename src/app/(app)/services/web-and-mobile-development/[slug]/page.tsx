...
export default async function IndustryPage({ params, searchParams }: PageProps) {
  ...
  if (slug.includes("_")) {
    ...
    redirect(`/services/web-and-mobile-development/${toCanonicalSlug(slug)}${langSuffix}`);
  }
  ...
  return (
    <>
      ...
      <div className="overflow-x-hidden" dir={isRTL ? "rtl" : "ltr"}>
        ...
        <Contact serviceName={data.hero_section.title} sourcePage={`/services/web-and-mobile-development/${toCanonicalSlug(slug)}`} />
      </div>
      <Footer />
    </>
  );
