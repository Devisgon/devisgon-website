...
import { workflowData } from "@/data/loaders/cloud";
...
export default async function IndustryPage({ params, searchParams }: PageProps) {
  ...
  if (slug.includes("_")) {
    ...
    redirect(`/services/cloud/${toCanonicalSlug(slug)}${langSuffix}`);
  }
  ...
  return (
    <>
      ...
      <div className="overflow-x-hidden" dir={isRTL ? "rtl" : "ltr"}>
        ...
        <Contact serviceName={data.hero_section.title} sourcePage={`/services/cloud/${toCanonicalSlug(slug)}`} />
      </div>
      <Footer />
    </>
  );
