...
- Agriculture industry detail pages currently include `agricultural-equipment`, `agricultural-shelters`, `fertilizer-suppliers`, `grain-storage`, `pesticide-companies`, and `seed-companies` across all language folders, with matching Agriculture navbar links in `src/data/navbar.json` and localized `src/data/*_data/navbar.json` files.
- Food industry detail pages currently include `bakery`, `juice-bar`, `catering`, and `hotels` across all language folders, with matching Food navbar links in `src/data/navbar.json` and localized `src/data/*_data/navbar.json` files.
- Healthcare industry detail pages currently include `beauty-therapist`, `chiropractor`, `dentist`, `massage-therapist`, `myotherapist`, `optometry`, `osteopath`, `physiotherapist`, `podiatrist`, and `psychologist` across all language folders, with `beauty-therapist` resolving to the `beautytherapist.json` file alias.
- Professional industry detail pages include `barber`, `consulting`, `education`, `fuel-station`, `gym`, `hairdresser`, `hvac`, `legal-services`, and `travel-services`; Real Estate includes `accountant`, `architecture-design`, `commercial`, `insurance`, `property-management`, and `residential`; Trades includes `carpentry`, `cleaning`, `clothing`, `electrician`, `mechanic`, `painter`, `plumbing`, and `roofing`.
...
- Sitemap generation gives `/` daily priority and boosts AI/ML service detail URLs to priority `0.95` so key AI service pages are emphasized in canonical sitemap output.
