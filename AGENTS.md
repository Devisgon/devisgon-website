...
- `src/components/navbar.js` dynamically imports the flag-heavy language switcher with `ssr: false`; keep language-switcher changes isolated in `src/components/language_switch_component.tsx` so country flag SVGs do not join every public page's initial client bundle.
...
- Page-level standalone internal-link sections have been removed; public pages should rely on the navbar/footer plus contextual CTAs, cards, legal anchors, and form links, and any placeholder or dead `href` values found during page work should be replaced with a real related destination.
...
- Services navigation now includes expanded service detail links for AI/ML, automation, cloud/DevOps, data solutions, digital design, testing, and web/SaaS additions, with localized labels mirrored in every `src/data/*_data/navbar.json` file; AI Software Development, Agentic AI, AI Agent, AI Chatbot, AI Integration, and Computer Vision now use long SEO slugs from their JSON `slug` fields.
...
- `/blogs/[slug]` -> `src/app/(app)/blogs/[slug]/page.tsx` queries Payload by slug (published only), emits blog-title metadata with canonical `/blogs/<slug>`, auto-translates title + lexical rich text + recent-post titles to selected language, and renders translated lexical rich text.
...
- Route behavior:
  ...
  - pulls content by scanning the service loader maps in `src/app/(app)/services/[slug]/page.tsx` and matching the requested slug against each imported JSON file's `slug` field, with English fallback when localized content is missing,
...
- Home/services/privacy/terms pages use language-specific JSON maps in route files; homepage service carousel cards are mirrored across every `src/data/*_data/home_page.json` file with shared asset paths, and `services_page.contact_form` provides the services-page bottom CTA heading/description.
...
- Service detail pages scan loader files in `src/data/loaders/*.ts` that map `{lang -> {loaderKey -> json}}`; the page matches public URLs directly to each JSON file's `slug` field, while loader keys remain only a direct compatibility fallback for imported records; AI/ML content is sourced from `services/ai_and_ml` and Web/SaaS content is sourced from `services/web_and_saas_development`; optional JSON `seo_metadata` (`title`, `description`, `keywords`) is mapped into page metadata with English/fallback defaults when absent.
- Service detail loader maps include the expanded English and localized service JSON files for AI/ML, automation, cloud/DevOps, data, design, testing, and web/SaaS pages; public service slugs should live in JSON `slug` fields, and removed English service files should be removed from localized folders too so loader imports and sitemap crawling stay aligned.
...
- Industry detail pages use flat public routes plus `src/data/*_data/industries/<category>/<slug>.json` and `src/data/loaders/industries.ts` filesystem loaders; JSON files may keep short filenames, but public `slug` fields and navbar/listing hrefs use SEO-style long slugs where replacements are available.
- Agriculture industry detail pages currently include SEO public slugs for agricultural equipment, agricultural shelters, fertilizer suppliers, grain storage, pesticide companies, and seed companies across all language folders, with matching Agriculture navbar links in `src/data/navbar.json` and localized `src/data/*_data/navbar.json` files.
- Food industry detail pages currently include SEO public slugs for bakery, juice bar, catering, and hotels across all language folders, with matching Food navbar links in `src/data/navbar.json` and localized `src/data/*_data/navbar.json` files.
- Healthcare industry detail pages currently include SEO public slugs for beauty therapist, chiropractor, dentist, massage therapist, myotherapist, optometry, osteopath, physiotherapist, podiatrist, and psychologist across all language folders, with old short slugs resolving through `src/data/loaders/industries.ts`.
- Professional industry detail pages include SEO public slugs for barber, consulting, education, fuel station, gym, hairdresser, HVAC, legal services, and travel services; Real Estate includes SEO public slugs for accountant, architecture/design, commercial, insurance, property management, and residential; Trades includes SEO public slugs for carpentry, cleaning, electrician, mechanic, and painter, while `clothing`, `plumbing`, and `roofing` remain on their existing short slugs until replacements are provided.
...
- Technology detail pages use `src/data/*_data/technologies/<category>/<slug>.json` and `src/data/loaders/technologies.ts` filesystem loaders; newly added English-only technology detail files fall back to English until localized copies are generated.
...
- Country dropdown options are generated in `src/lib/inquiry-options.ts` from a local ISO code list and `Intl.DisplayNames`; do not reintroduce `country-flag-icons` there because the form select only needs country labels.
- Data-driven React icons are resolved through the allowlisted `src/components/icons.js` registry. Add concrete named imports for new JSON icon names instead of namespace-importing whole `react-icons` families.
- Technologies data is organized by main category folders:
  - `technologies/automation/*.json` automation tool files (`google_app_script.json`, `hubspot.json`, `make.json`, `microsoft_power_automation.json`, `n8n.json`, `zapier.json`) that resolve to public slugs `google-apps-script`, `hubspot`, `make`, `microsoft-power-automate`, `n8n`, and `zapier`,
  - `technologies/crms/*.json` CRM/CMS files (`ghl.json`, `jira.json`, `microsoft_dynamo.json`, `odoo.json`, `payload.json`) that resolve to public slugs `gohighlevel`, `jira-crm`, `microsoft-dynamics-365-crm`, `odoo`, and `payload-cms`,
...
- Industries dropdown links are grouped in `src/data/navbar.json` into six columns (`Agriculture`, `Food`, `Healthcare`, `Professional`, `Real Estate`, `Trades`) with 43 flat `/industries/<slug>` sub-industry links. The former Entertainment column remains removed; current moved/new items include `hotels` under Food, `clothing` under Trades, healthcare additions, professional additions, `accountant` under Real Estate, and `mechanic`/`painter` under Trades. SEO replacements resolve in `src/data/loaders/industries.ts`; `/industries/ai-automation-software-solutions` redirects to the main `/industries` listing because it represents the broad industry-solutions page rather than a single detail file.
...
- Technologies dropdown links in `src/data/navbar.json` and localized navbar files point directly to dedicated subcategory routes under `/technologies/[slug]` (for example `google-apps-script`, `hubspot`, `make`, `microsoft-power-automate`, `n8n`, `zapier`, `gohighlevel`, `jira-crm`, `microsoft-dynamics-365-crm`, `odoo`, `payload-cms`, `javascript-typescript`, `nextjs-nodejs`, `java`, `python`, `ruby`, `rust`, `react-native`, `nestjs`, `mongodb`, `amazon-dynamodb`, `doctorhosters`, `shopify`, `wordpress`).
...
- Service detail loaders must import the JSON files used by `/services/[slug]`; public nav slugs should match the JSON `slug` values directly.
...
- Service slug in JSON -> direct service route lookup + `navbar.json` link + sitemap output; public service URLs should use lowercase hyphenated `/services/<slug>` paths, not category-prefixed paths, spaces, capitals, or underscores.
...
- 2026-05-25: Standardized public page canonical Metadata API output, added missing canonical metadata for get-started, partner, and blog detail pages, and made route canonical paths override JSON canonical values.
- 2026-05-25: Shortened SEO titles to a 55-character maximum across metadata and JSON SEO sources, kept canonical English SEO titles/descriptions unique, and added a runtime title compaction guard.
- 2026-05-26: Replaced the Agriculture industry set with agriculture equipment, shelter, fertilizer, grain storage, greenhouse/nursery, pesticide, and seed company detail pages across language folders, refreshed navbar/listing references, and updated the industry loader slugs.
- 2026-06-01: Removed the Industries Entertainment column, moved its linked detail JSON into Food, Professional, and Trades categories, deleted unlinked industry JSON files, and synced navbar/listing/loader slugs across language folders.
- 2026-06-01: Pruned the industry dataset again to match the latest navbar, removing fine dining, restaurants, supermarket, events, media production, tutor, electronics, and welding JSON, and remapped stale explore links to active industry slugs.
- 2026-06-01: Strengthened homepage and AI/ML service SEO signals with richer homepage metadata, Organization/WebPage/Service/Breadcrumb/FAQ JSON-LD, and sitemap priority boosts for AI/ML service URLs.
- 2026-06-01: Translated the newly added English industry JSON files into localized language folders, added matching localized navbar links, refreshed the English industries listing, and updated the industry loader for the new healthcare, professional, real estate, and trades slugs.
- 2026-06-02: Added new Automation and CRM technology links to English and localized navbars, wired their technology loader slugs, and documented the expanded technology source set.
- 2026-06-02: Mirrored new homepage service carousel cards for AI Agents, AI Chatbot, and LLM Prompt Engineering across localized homepage JSON files.
- 2026-06-02: Added a shared contextual internal-link section across major public page templates, including listing pages, detail pages, partners, forms, legal pages, blogs, process, and 404 recovery.
- 2026-06-03: Removed the shared standalone internal-link component from public pages, deleted the unused component file, and replaced the privacy-page placeholder location link with a working map link.
- 2026-06-03: Updated provided service and industry public URLs to long SEO slugs across localized JSON/navbar data, added shared old-to-new slug aliases, and routed the generic industry-solutions alias back to `/industries`.
- 2026-06-03: Removed the separate URL slug alias helper file and kept SEO slug resolution inside existing service and industry route/loader files.
- 2026-06-03: Updated the remaining AI software, agentic AI, AI agent, AI chatbot, AI integration, and computer vision service pages to the provided long SEO slugs across localized JSON/navbar data, homepage links, and sitemap priority entries.
