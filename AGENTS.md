...
- Technologies is nested inside the About dropdown in `src/data/navbar.json` and localized navbar files, not rendered as a top-level navbar item. In the desktop/mobile navbar, clicking Technologies inside About replaces the About dropdown content with a normal rounded Technologies dropdown where each bold category label is followed by colon-separated bold inline links divided by pipes; footer, service technology cards, and `/technologies` use `findNavbarItemByHref` to find that nested item.
- Desktop navbar dropdown triggers preview on hover, pin open on single click, stay open when the cursor leaves while pinned, close on outside click/Escape/sublink navigation, and only navigate to the top-level page on double-click; the global navigation progress listener ignores the first click on these dropdown triggers and starts only when the double-click navigation is allowed.
